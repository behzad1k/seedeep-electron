...
export const CameraFeed = memo<CameraFeedProps>(({
  ...
  const connectWebSocket = useCallback(() => {
    ...
    console.log(`[CameraFeed ${normalizedCameraId}] Connecting to WebSocket:`, wsFullUrl);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        console.error(`[CameraFeed ${normalizedCameraId}] Error parsing WebSocket message:`, err);
      }
    };
    ...
    wsRef.current = ws;
  }, [normalizedCameraId, wsUrl, renderDetections, onFrame, onError, stats.fps]);

  // Start authenticated MJPEG stream
  const startAuthenticatedMJPEGStream = useCallback(async (url: string, username: string, password: string) => {
    if (!videoCanvasRef.current || !mountedRef.current) return;

    const canvas = videoCanvasRef.current;
    const ctx = canvas.getContext('2d');
    if (!ctx) {
      console.error(`[CameraFeed ${normalizedCameraId}] Failed to get canvas context`);
      return;
    }

    console.log(`[CameraFeed ${normalizedCameraId}] Starting MJPEG stream from:`, url, password);
    ...
    try {
      ...
      console.log(`[CameraFeed ${normalizedCameraId}] Stream connected, status: ${response.status}`);
      console.log(`[CameraFeed ${normalizedCameraId}] Content-Type:`, response.headers.get('content-type'));
      ...
      while (mountedRef.current) {
        ...
        if (startIndex !== -1 && endIndex !== -1) {
          ...
          img.onload = () => {
            ...
            if (canvas.width !== img.width || canvas.height !== img.height) {
              ...
              console.log(`[CameraFeed ${normalizedCameraId}] Canvas size set to ${img.width}x${img.height}`);
          ...
          img.onerror = () => {
            console.error(`[CameraFeed ${normalizedCameraId}] Failed to load JPEG frame`);
            URL.revokeObjectURL(imageUrl);
          };
        ...
        if (buffer.length > 1024 * 1024) { // 1MB
          console.warn(`[CameraFeed ${normalizedCameraId}] Buffer too large, clearing...`);
          buffer = new Uint8Array(0);
        }
      }

      console.log(`[CameraFeed ${normalizedCameraId}] Stream reading loop ended`);
      ...
      if (err.name === 'AbortError') {
        ...
        console.error(`[CameraFeed ${normalizedCameraId}] Stream error:`, err);
  ...
  const drawAnnotations = useCallback(() => {
    if (!annotationCanvasRef.current || !videoCanvasRef.current || !mountedRef.current) {
      return;
    }
    ...
    if (canvas.width !== videoCanvas.width || canvas.height !== videoCanvas.height) {
    ...
    const annotation = annotationQueueRef.current.getForTimestamp(videoCurrentTime);
  ...
  useEffect(() => {
    ...
    const initializeCamera = async () => {
      try {
        console.log(`[CameraFeed ${normalizedCameraId}] Initializing camera...`);

        const response = await fetch(`http://localhost:8000/api/v1/cameras/${normalizedCameraId}`);
        if (!response.ok) {
        ...
        console.log(`[CameraFeed ${normalizedCameraId}] Camera config:`, camera);
        console.log(`[CameraFeed ${normalizedCameraId}] Stream URL:`, camera.rtsp_url);
        ...
        console.log(`[CameraFeed ${normalizedCameraId}] Clean URL:`, cleanUrl);
        console.log(`[CameraFeed ${normalizedCameraId}] Username:`, username);
        console.log(`[CameraFeed ${normalizedCameraId}] Password length:`, password.length);
        ...
        console.error(`[CameraFeed ${normalizedCameraId}] Initialization error:`, err);
  ...
  }, [normalizedCameraId, isVisible, startAuthenticatedMJPEGStream, connectWebSocket, drawAnnotations, onError]);
      ...
      {error && (
        <Box sx={{
          ...
          <Typography variant="caption" sx={{ mt: 2, display: 'block', color: '#888' }}>
            Check camera credentials, network connectivity, and camera settings
          </Typography>
        </Box>
      )}
          ...
          <Box sx={{
            ...
            {stats.latency > 0 && (
              <Typography
                ...
                sx={{
                  ml: 1,
                  color: stats.latency > 500 ? '#f44336' : stats.latency > 300 ? '#ff9800' : '#4caf50',
