import { BackendCamera, BackendCameraFeature, BackendCameraFeatures, Camera } from '@shared/types';
...
type FeatureLabels = { key: BackendCameraFeature, label: string, desc: string }
// Camera Control Sidebar Component
const CameraControlSidebar = ({ camera, onClose, onUpdate, darkMode }: Props) => {
  ...
  const [calibrationPoints, setCalibrationPoints] = useState<{ pixel_x: number, pixel_y: number, real_x: number, real_y: number }[]>([]);
  ...
  const toggleFeature = (feature: BackendCameraFeature) => {
    setConfig(prev => ({
      ...prev,
      features: {
        ...prev.features,
  ...
  const addCalibrationPoint = (pixelX: number, pixelY: number, realX: number, realY: number) => {
  ...
  const submitCalibration = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/cameras/${camera.id}/calibrate`, {
        method: 'POST',
  ...
  const modelCardStyle = (isActive: boolean, color: string) => ({
    padding: '12px',
    marginBottom: '12px',
  ...
  const switchStyle = (checked: boolean): CSSProperties => ({
    position: 'relative',
    width: '48px',
    height: '24px',
          ...
          {activeTab === 'tracking' && (
            <div>
              ...
              <div style={{ marginBottom: '24px' }}>
                ...
                {([
                  { key: 'detection', label: 'Object Tracking', desc: 'Track objects across frames with unique IDs' },
