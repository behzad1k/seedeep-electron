...
const MainLayout: React.FC = () => {
  ...
  const [gridSize, setGridSize] = useState<"2x2" | "3x3" | "4x4" | "5x5">(
    "3x3",
  );
  const [addCameraOpen, setAddCameraOpen] = useState(false);
  const [rightSidebarOpen, setRightSidebarOpen] = useState(false);
  const [expandedCamera, setExpandedCamera] = useState<BackendCamera | null>(
    null,
  );
  ...
  const handleUpdate = useCallback(
    async (cameraId: string, updates: any) => {
      ...
      const updatePayload = {
        ...
        active_models:
          updates.active_models || currentCamera?.detectionModels
  ...
  return (
    <Box
      sx={{ paddingLeft: 7, bgcolor: "background.default", minHeight: "100vh" }}
    >
      ...
      {memoryStatus.isCritical && (
        <Alert
          severity="error"
          sx={{ position: "fixed", top: 60, right: 16, zIndex: 9999 }}
        >
