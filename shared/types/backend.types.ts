...
export type BackendCameraFeature = 'detection' | 'tracking' | 'speed' | 'counting'
 ...
 */
export function isTrackingResult(value: unknown): value is BackendTrackingResult {
  if (!value || typeof value !== 'object') return false;
  const obj = value as any;
  return (
    obj.tracked_objects &&
...
export function parseWebSocketResponse(data: BackendWebSocketResponse) {
  ...
  return {
    ...
    tracking: trackingData ? {
      trackedObjects: Object.values((trackingData as BackendTrackingResult).tracked_objects),
 ...
 */
export function extractDetections(data: BackendWebSocketResponse): BackendDetection[] {
 ...
 */
export function extractTracking(data: BackendWebSocketResponse): BackendTrackingResult | null {
 ...
 */
export function getDetectionCountsByModel(data: BackendWebSocketResponse): Record<string, number> {
...
export function getTotalDetectionCount(data: BackendWebSocketResponse): number {
  return Object.values(getDetectionCountsByModel(data)).reduce((sum, count) => sum + count, 0);
}