		...
		try {
			...
			if (fs.existsSync(backendDir)) {
				...
				files.forEach((file) => {
					...
					console.log(
						`   ${stats.isDirectory() ? "📁" : "📄"} ${file} (${permissions})`,
					);
				});
				...
				if (fs.existsSync(startScript)) {
					...
					const scriptPerms = (scriptStats.mode & parseInt("777", 8)).toString(
						8,
					);
				...
				if (fs.existsSync(pythonEnv)) {
					...
					console.log(
						`🐍 Python binary: ${fs.existsSync(pythonBin) ? "EXISTS" : "MISSING"}`,
					);

					if (fs.existsSync(pythonBin)) {
						const pythonStats = fs.statSync(pythonBin);
						const pythonPerms = (
							pythonStats.mode & parseInt("777", 8)
						).toString(8);
				...
				console.log(
					`📄 main.py: ${fs.existsSync(mainPy) ? "EXISTS" : "MISSING"}`,
				);
			} else {
	...
	async start(): Promise<boolean> {
		...
		try {
			...
			this.backendProcess.on("exit", (code, signal) => {
				console.log(
					`⚠️ Backend process exited - code: ${code}, signal: ${signal}`,
				);
